...
//			printf("**************************************************************\n");
		...
		if (!dsp_node_allocate(dsp_handle, base->proc, codec->uuid, arg_data, &attrs, &node)) {
			pr_err(self, "dsp node allocate failed hiero");
			free(arg_data);
			return NULL;
		}
...
//this is where to something goes wrong with memory

/*printf("Gaat fout hier\n");
printf("node.msgbuf_size: %d\n", node->msgbuf_size);
printf("node.msgbuf_addr *: %d\n", node->msgbuf_addr);
printf("node.msgbuf_addr : %d\n", &(node->msgbuf_addr));
...
//return NULL; // tmp
	int tmpres = dsp_node_create(dsp_handle, node);
//	printf("tmpres: %d", tmpres);
	
	if (!tmpres) {
		pr_err(self, "dsp node create failed hoi!!!");
...
skip_setup:
	...
	if (base->node)
		return TRUE;
//printf("Cullprit function\n");
	base->node = create_node(self); // in here is the cullprit
	if (!base->node) {
